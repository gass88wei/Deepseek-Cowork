...
import { existsSync, mkdirSync, readFileSync, constants, unlinkSync, writeFileSync, readdirSync, statSync } from 'node:fs';
  ...
  instance.interceptors.response.use(
    ...
    (error) => {
      if (error.response?.status === 401) {
        ...
        if (errorMessage.toLowerCase().includes("api key") || errorMessage.toLowerCase().includes("api-key") || errorMessage.toLowerCase().includes("x-api-key")) {
          logger.debug("[HTTP] API Key authentication failed:", errorMessage);
          return Promise.reject(new ApiKeyAuthError(
            `API Key authentication failed: ${errorMessage}. Please check your API Key configuration using "happy config show" or set HAPPY_API_KEY environment variable.`
          ));
        }
        logger.debug("[HTTP] Token authentication failed:", errorMessage);
        return Promise.reject(new TokenAuthError(
          `Authentication failed: ${errorMessage || "Invalid or expired token"}. Please try "happy auth login --force" to re-authenticate.`
        ));
      }
      return Promise.reject(error);
    }
...
function getSocketExtraHeaders() {
  const headers = {};
  if (configuration.apiKey) {
    headers["X-API-Key"] = configuration.apiKey;
  }
...
export { ApiClient as A, RawJSONLinesSchema as R, ApiSessionClient as a, packageJson as b, configuration as c, backoff as d, delay as e, AsyncLock as f, readDaemonState as g, clearDaemonState as h, readCredentials as i, encodeBase64 as j, httpClient as k, logger as l, ApiKeyAuthError as m, encodeBase64Url as n, decodeBase64 as o, projectPath as p, writeCredentialsDataKey as q, readSettings as r, acquireDaemonLock as s, writeDaemonState as t, updateSettings as u, releaseDaemonLock as v, writeCredentialsLegacy as w, clearCredentials as x, clearMachineId as y, getLatestDaemonLog as z };
