...
class SessionManager extends EventEmitter {
    ...
    async createSession(name, workDirPath, options = {}) {
        ...
        try {
            ...
            // 使用统一的 _setSession 方法记录失败状态（自动维护 workDirIndex）
            const failedSessionInfo = {
                workDir: resolvedPath,
                status: 'failed',
                error: error.message,
                createdAt: new Date().toISOString()
            };
            this._setSession(name, failedSessionInfo, oldWorkDir);
        ...
        if (!wasDaemonRunning) {
            ...
            if (needsCleanup) {
                ...
                if (existingState) {
                    ...
                    if (validation.invalid.length > 0 || validation.failed.length > 0) {
                        logInfo(`Found ${validation.invalid.length} invalid and ${validation.failed.length} failed sessions, will recreate them`);
                    }
                } else {
    ...
    async createSessionForWorkDir(workDir) {
        ...
        if (existingName && this.sessions[existingName]) {
            ...
            // 如果 session 有 sessionId 且不是失败状态，验证是否仍然有效
            if (existingSession.sessionId && existingSession.status !== 'failed') {
                ...
                // session 是失败状态或没有 sessionId，需要重新创建
                logInfo(`Session "${existingName}" was in failed state, will recreate`);
            }
        }
     ...
     * - 正向：验证本地 sessions 在 daemon 中是否有效
     * - 反向：发现 daemon 中存在但本地没有的孤儿 session，冲突则清理，不冲突则补充
     * @returns {Promise<Object>} { valid, invalid, failed, orphans: { added, removed } }
     */
    async validateSessions() {
        const result = {
            valid: [],    // 在 daemon 中存在的 session
            invalid: [],  // 在 daemon 中不存在的 session
            failed: [],   // 状态为 failed 的 session
            orphans: {
        ...
        if (localSessionCount > 0) {
            ...
            for (const [name, session] of Object.entries(this.sessions)) {
                ...
                if (session.status === 'failed') {
                    result.failed.push({ name, session });
                    continue;
                }
                
                // 检查是否有 sessionId
                if (!session.sessionId) {
                    result.invalid.push({ name, session, reason: 'no sessionId' });
                    continue;
                }
                ...
                if (daemonSession) {
                    ...
                    result.invalid.push({ name, session, reason: 'not found in daemon' });
                    session.status = 'inactive';
                }
            }
        }
        
        // 5. 反向对账：找出 daemon 中的孤儿 session（daemon 有但本地没有）
        ...
        logInfo(`Validation complete: ${result.valid.length} valid, ${result.invalid.length} invalid, ${result.failed.length} failed, ${result.orphans.added.length} adopted, ${result.orphans.removed.length} removed`);
    ...
    /**
     * 清理无效的 sessions
     * 删除 daemon 中不存在或状态为 failed 的 session 记录
     * @param {Object} options 选项
     * @param {boolean} options.keepFailed 是否保留 failed 状态的记录（默认 false）
     * @returns {Promise<Object>} { cleaned: number, details: [...] }
     */
    async cleanupInvalidSessions(options = {}) {
        const { keepFailed = false } = options;
        
        logInfo('Starting cleanup of invalid sessions...');
        ...
        // 清理 invalid sessions
        for (const { name, reason } of validation.invalid) {
            const removed = this._removeSession(name);
            if (removed) {
                cleaned.push({ name, reason, type: 'invalid' });
                logInfo(`Cleaned up invalid session: ${name} (${reason})`);
            }
        }
        
        // 可选：清理 failed sessions
        if (!keepFailed) {
            for (const { name } of validation.failed) {
                const removed = this._removeSession(name);
                if (removed) {
                    cleaned.push({ name, reason: 'failed status', type: 'failed' });
                    logInfo(`Cleaned up failed session: ${name}`);
                }
            }
        }
        
        // 保存状态（跳过同步，因为上面已经调用过 validateSessions）
        if (cleaned.length > 0) {
            await this.saveStateFile({ skipSync: true });
        }
        
        logInfo(`Cleanup complete: ${cleaned.length} sessions removed`);
