...
class ConversationManager extends EventEmitter {
  ...
  /**
   * 处理 agentState 更新（权限请求）
   * 当收到包含 agentState 的更新时，解密并发出事件
   * @param {Object} body - WebSocket update 消息体
   * @private
   */
  _handleAgentStateUpdate(body) {
    // 检查是否包含 agentState 更新
    if (!body || !body.agentState) {
      return;
    }
    ...
    // 检查版本是否更新
    const newVersion = body.agentState.version;
    if (newVersion <= this.agentStateVersion) {
      return; // 版本未更新
    }
    ...
    try {
      ...
      if (!enc) {
        console.warn('[ConversationManager] No encryption for agentState');
        return;
      }
      
      // 解密 agentState
      let decryptedAgentState = null;
      if (body.agentState.value) {
        const encryptedData = CryptoUtils.decodeBase64(body.agentState.value, 'base64');
        decryptedAgentState = this.encryption.decrypt(enc, encryptedData);
      }
      
      // 更新本地状态
      const oldAgentState = this.agentState;
      this.agentState = decryptedAgentState;
      this.agentStateVersion = newVersion;
      
      console.log(`[ConversationManager] AgentState updated: version=${newVersion}, hasRequests=${!!decryptedAgentState?.requests}`);
      
      // 检查是否有新的权限请求
      const hasNewRequests = decryptedAgentState?.requests && 
        Object.keys(decryptedAgentState.requests).length > 0;
      
      if (this.options.debug) {
        console.log('[ConversationManager] AgentState:', JSON.stringify(decryptedAgentState, null, 2));
      }
      
      // 发出 agentState 更新事件
      this.emit('agentStateUpdated', {
        sessionId,
      ...
      console.error('[ConversationManager] Failed to decrypt agentState:', error.message);
    }
  }
  ...
  _handleWebSocketUpdate(data) {
    try {
      ...
      // 处理 agentState 更新（权限请求）
      this._handleAgentStateUpdate(body);
      
      // 对于 update-session 类型的事件，只记录日志，不处理（agentState 已在上面处理）
