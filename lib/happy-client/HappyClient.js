...
class HappyClient extends EventEmitter {
  ...
  _setupEventForwarding() {
    ...
    if (this.conversationManager) {
      ...
      // 转发 agentState 更新事件（权限请求）
      // 当收到权限请求时，前端需要显示确认对话框
      this.conversationManager.on('agentStateUpdated', (event) => {
        this.emit('agentStateUpdated', event);
      });
    }
  }
   ...
   */
  getAgentState() {
    return this.conversationManager?.getAgentState() || null;
  }
